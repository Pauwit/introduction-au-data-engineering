...
class ContactsRepositorySpec extends AnyFlatSpec with Matchers {
  ...
  "load" should "read contacts from a data lake file keyed by geohash" in {
    val contacts = ContactsRepository.load("src/test/resources/test-contacts.json")

    contacts should have size 2
  }

  it should "return an empty map for a missing file" in {
