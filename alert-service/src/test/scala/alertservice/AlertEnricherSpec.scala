...
class AlertEnricherSpec extends AnyFlatSpec with Matchers {
  ...
  "enrich" should "attach owner and contact when the geohash matches a known zone" in {
    val alert = Alert(timestamp, "drone-001", 57.64911, 10.40744, 60.0, 70.0, 950.0, 15.0, "high temperature and smoke")
  ...
  it should "default owner and contact to unknown when the zone has no match" in {
    val alert = Alert(timestamp, "drone-002", 0.0, 0.0, 60.0, 70.0, 950.0, 15.0, "high temperature and smoke")
    val contacts = Map("u4pru" -> Contact("Office National des Forets - Vosges", "+33 3 88 00 11 22"))
  ...
  it should "preserve the original alert fields" in {
    val alert = Alert(timestamp, "drone-003", 43.5, 5.2, 60.0, 70.0, 950.0, 15.0, "high temperature and smoke")
    ...
    enriched.temperature shouldEqual 60.0
    enriched.smoke shouldEqual 70.0
